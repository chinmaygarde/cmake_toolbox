...
def main():
  ...
  with open(args.source, 'rb') as source, open(output_source, 'w') as output:
    ...
    output.write(
        f'alignas(std::max_align_t) const unsigned char xxd_{args.symbol_name}_data[] =\n'
    )
    output.write('{\n')
    while True:
      byte = source.read(1)
    ...
    output.write('};\n')
    output.write(f'const unsigned long xxd_{args.symbol_name}_length = {data_len};\n')
  ...
  with open(output_header, 'w') as output:
    ...
    output.write(f'extern const unsigned char xxd_{args.symbol_name}_data[];\n')
    output.write(f'extern const unsigned long xxd_{args.symbol_name}_length;\n\n')
